...
const SINCOS_PRECISION: f32 = 0.5;
const SINCOS_LENGTH: u32 = @divTrunc(360, SINCOS_PRECISION);
var cosLUT: [SINCOS_LENGTH]f32 = undefined;
const DEG_TO_RAD: f32 = 3.141592653589793 / 180.0;

const perlin_PI = @as(f32, @floatFromInt(SINCOS_LENGTH >> 1));

var initialized: bool = false;

inline fn noise_fsc(i: f32) f32 {
...
pub const PerlinGenerator = struct {
    ...
    falloff: f32 = 0.5, // 50% reduction/octave
    perlin: ?[]f32 = null,
    ...
    pub fn init(allocator: *const std.mem.Allocator, seed: u32) !PerlinGenerator {
        if (!initialized) {
            ...
            while (i < SINCOS_LENGTH) : (i += 1) {
                ...
                cosLUT[i] = std.math.cos(if32 * DEG_TO_RAD * SINCOS_PRECISION);
            }

            initialized = true;
        }

        var generator = PerlinGenerator{ .allocatorPtr = allocator };
    ...
    pub fn seedNoise(self: *PerlinGenerator, seed: u32) !void {
        ...
        if (self.perlin == null) {
            var allocator = self.allocatorPtr.*;
            self.perlin = try allocator.alloc(f32, PERLIN_SIZE + 1);
        }
        ...
        while (i < PERLIN_SIZE + 1) : (i += 1) {
            ...
            self.perlin.?[i] = @as(f32, @floatFromInt(z)) / @as(f32, @floatFromInt(m));
        }
    }

    pub fn get(self: *PerlinGenerator, x_: f32, y_: f32, z_: f32) f32 {
        ...
        var rxf: f32 = undefined;
        var ryf: f32 = undefined;
...
pub export fn noise1(x: f32) f32 {
    return globalGenerator.?.get(x, 0.0, 0.0);
}

pub export fn noise2(x: f32, y: f32) f32 {
    return globalGenerator.?.get(x, y, 0.0);
}

pub export fn noise3(x: f32, y: f32, z: f32) f32 {
    return globalGenerator.?.get(x, y, z);
}

pub export fn noiseDetail(octaves: i32, falloff: f32) void {
